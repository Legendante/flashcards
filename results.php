...
$Sessions = getStudySessions($UserID, $SetID, $_SESSION['parentuserid'], 0);
$Results = getStudySessionResults($UserID, $SetID, $_SESSION['parentuserid'], 0);
...
if($_SESSION['userid'] == $_SESSION['parentuserid'])
{
	...
	echo "<p><a href='results.php?s=" . $SetID . "&u=" . $_SESSION['userid'] . "'>Mine</a></p>";
	foreach($Students AS $StudentID => $StudentRec)
	{
		echo "<p><a href='results.php?s=" . $SetID . "&u=" . $StudentID . "'>" . $StudentRec['firstname'] . " " . $StudentRec['lastname'] . "</a></p>";
	}
	echo "</div>";
	...
	if(isset($Results[$SessionID]))
	{
		foreach($Results[$SessionID] AS $Cnt => $Result)
		{
			if(isset($Cards[$Result['cardid']]))
			{
				...
				if($Cards[$Result['cardid']]['manualanswer'] == 0)
					$GivenAnswer = '***';
